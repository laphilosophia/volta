...
- **useVoltaComponent**: Fixed infinite render loop caused by unstable `props` reference in `useCallback` dependency array. Now uses `useMemo` with `JSON.stringify` to stabilize props.
